...
function isTelegramTokenFormat(token?: string | null): boolean {
  if (!token || typeof token !== "string") return false;
  const trimmed = token.trim();
  if (
    trimmed.includes("put-your") ||
    trimmed.includes("placeholder") ||
    trimmed.includes("your-token") ||
    trimmed.includes("your-bot-token") ||
    trimmed.includes("your_bot_token") ||
    trimmed.includes("example")
  ) {
    return false;
  }
  return /^\d{6,13}:[A-Za-z0-9_-]{25,60}$/.test(trimmed);
}

function hasActiveBotToken(): boolean {
  const murad = getMuradBotConfig();
  if (murad && isTelegramTokenFormat(murad.botToken)) return true;
  return isTelegramTokenFormat(process.env.TELEGRAM_BOT_TOKEN);
}

function getActiveBotToken(): string {
  const murad = getMuradBotConfig();
  if (murad && murad.botToken && isTelegramTokenFormat(murad.botToken)) return murad.botToken.trim();
  const token = process.env.TELEGRAM_BOT_TOKEN;
  if (token && isTelegramTokenFormat(token)) {
    return token.trim();
  }
  if (murad && murad.botToken) return murad.botToken.trim();
...
function getSlotPorts(slot: string) {
  if (slot === "B") {
    return { vless: 10090, vmess: 10091, trojan: 10092, shadowsocks: 10093 };
  }
  return { vless: 10080, vmess: 10081, trojan: 10082, shadowsocks: 10083 };
}

function getShadowsocksPortForClient(slot: "A" | "B", clientId: string, allClients: ClientConfig[]): number {
  if (clientId === "default-shadowsocks") {
    return slot === "B" ? 10093 : 10083;
  }
  const basePort = slot === "B" ? 10100 : 10000;
  const ssList = allClients.filter(c => c.protocol === "shadowsocks" && c.id !== "default-shadowsocks");
  const idx = ssList.findIndex(c => c.id === clientId);
  if (idx === -1) {
    return slot === "B" ? 10093 : 10083;
  }
  return basePort + idx;
...
function generateRandomShadowsocksPassword(): string {
  ...
  if (!fs.existsSync(clientsFilePath)) {
    ...
    const defaultDB = {
      clients: [
        ...
        {
          id: "default-shadowsocks",
          name: "Default Client (Shadowsocks)",
          protocol: "shadowsocks",
          uuid: generateRandomShadowsocksPassword(),
          path: "/ss",
          limitGB: 0,
          consumedUpload: 0,
          consumedDownload: 0,
    ...
    addLog("Created initial clients.json database (default VLESS + VMess + Trojan + Shadowsocks clients).");
    return;
  }

  // Migration: a clients.json created before this change only has the
  // default VLESS client. Seed the missing default VMess/Trojan/Shadowsocks clients
  // once so upgraded deployments also get permanent default configs for
  // all protocols, without touching any existing user clients.
  try {
    ...
    if (!existingClients.some((c) => c.id === "default-shadowsocks")) {
      existingClients.push({
        id: "default-shadowsocks",
        name: "Default Client (Shadowsocks)",
        protocol: "shadowsocks",
    ...
    // Automatically migrate any old shadowsocks clients that still had @dxuser or @dx prefix, or old /ss_ paths
    for (const c of existingClients) {
      if (c.protocol === "shadowsocks" && (!c.uuid || c.uuid === "@dxuser" || c.uuid.startsWith("@dx"))) {
        c.uuid = generateRandomShadowsocksPassword();
        changed = true;
      }
      if (c.protocol === "shadowsocks" && c.path && c.path.startsWith("/ss_")) {
        c.path = c.path.replace("/ss_", "/by_moon_");
        changed = true;
      }
    }
    ...
    if (changed) {
      ...
      addLog("Migrated clients.json: added missing default VMess/Trojan/Shadowsocks clients (random shadowsocks password).");
    }
  } catch (e: any) {
...
function generateV2RayConfigForSlot(slot: "A" | "B", clients: ClientConfig[]) {
  ...
  const ssClients = activeClients.filter(c => c.protocol === "shadowsocks");
  ...
  // Add Shadowsocks default inbound (/ss)
  const defaultSsClient = ssClients.find(c => c.id === "default-shadowsocks");
  const defaultSsPassword = (defaultSsClient && defaultSsClient.uuid && defaultSsClient.uuid !== "@dxuser")
    ? defaultSsClient.uuid
    : generateRandomShadowsocksPassword();
  const defaultSsPath = defaultSsClient ? defaultSsClient.path : "/ss";
  ...
  inbounds.push({
    ...
    settings: {
      method: "aes-128-gcm",
      password: defaultSsPassword,
      network: "tcp,udp"
    },
    streamSettings: {
      network: "ws",
      wsSettings: { path: defaultSsPath, headers: wsHeaders },
      sockopt: { tcpFastOpen: true, tcpKeepAliveInterval: 15 }
    }
  });

  // For any custom Shadowsocks clients, allocate individual inbound ports
  for (const c of ssClients) {
    if (c.id === "default-shadowsocks") continue;
    const clientPort = getShadowsocksPortForClient(slot, c.id, activeClients);
    inbounds.push({
      listen: "0.0.0.0",
      port: clientPort,
      protocol: "shadowsocks",
      ...
      streamSettings: {
        network: "ws",
        wsSettings: { path: c.path, headers: wsHeaders },
        sockopt: { tcpFastOpen: true, tcpKeepAliveInterval: 15 }
      }
    });
  }
...
function buildDarkTunnelLink(client: ClientConfig): string {
  ...
  if (client.protocol === "shadowsocks") {
    const v2rayCustomConfig = generateShadowsocksClientJson(client);
    const payload = {
      type: "V2RAY_CUSTOM_CONFIG",
      name: client.name || "shadowsocks",
      v2rayCustomConfigTunnelConfig: {
        v2rayCustomConfig
      }
    };
...
async function sendAppConfigZip(chatId: number | string, client: ClientConfig) {
  try {
    const isSS = client.protocol === "shadowsocks";
    const safeName = (client.name || (isSS ? "shadowsocks" : "config"))
      .replace(/[\/\\:*?"<>|\x00-\x1F]+/g, "_")
      .replace(/\s+/g, "_")
      .slice(0, 60) || (isSS ? "shadowsocks" : "config");

    const fileContent = buildDarkTunnelLink(client);
    const fileName = (isSS && (client.id === "default-shadowsocks" || safeName.toLowerCase() === "shadowsocks"))
      ? "shadowsocks.dark"
      : `${safeName}.dark`;
    ...
    if (isSS) {
      const domain = getPublicDomain();
      caption =
        `📦 <b>ملف تكوين V2Ray Custom / DarkTunnel (${escapeHtml(client.name || "Shadowsocks")})</b>\n\n` +
        `📱 <b>طريقة التشغيل في تطبيق Dark Tunnel:</b>\n` +
        `1️⃣ افتح تطبيق <b>Dark Tunnel</b>.\n` +
        `2️⃣ اضغط استيراد (Import Config) ← اختر هذا الملف <code>${fileName}</code>.\n` +
        `💡 تم تضمين التكوين تلقائياً في خانة <b>V2Ray Custom</b> داخل DarkTunnel.\n\n` +
        `🌐 <b>الهوست (Host):</b> <code>${escapeHtml(domain)}</code>\n` +
        `🎯 <b>التمويه (SNI):</b> <code>YouTube.com</code>\n` +
        `🔑 <b>كلمة المرور:</b> <code>${escapeHtml(client.uuid)}</code>\n` +
        `📍 <b>المسار (Path):</b> <code>${escapeHtml(client.path || "/ss")}</code>`;
    } else {
      caption =
        `📦 <b>ملف تكوين DarkTunnel (${escapeHtml(client.name || "")})</b>\n\n` +
        `📱 افتح تطبيق DarkTunnel ← استيراد (Import) ← اختر هذا الملف <code>${fileName}</code>\n\n` +
...
function generateShadowsocksClientJson(client: ClientConfig, domainOverride?: string): string {
  const domain = domainOverride || getPublicDomain();
  const password = (client.uuid && client.uuid !== "@dxuser") ? client.uuid : generateRandomShadowsocksPassword();
  const clientPath = client.path || "/ss";

  const configObj = {
    inbounds: [
      {
        listen: "$SOCKS5_LISTEN_HOST",
        port: "$SOCKS5_LISTEN_PORT",
        protocol: "socks",
        settings: {
          auth: "noauth",
          udp: true,
...
async function sendShadowsocksJsonFile(chatId: number | string, client: ClientConfig) {
  try {
    const jsonStr = generateShadowsocksClientJson(client);
    const safeName = (client.name || "shadowsocks")
      .replace(/[\/\\:*?"<>|\x00-\x1F]+/g, "_")
      .replace(/\s+/g, "_")
      .slice(0, 60) || "shadowsocks";

    const domain = getPublicDomain();
    const password = (client.uuid && client.uuid !== "@dxuser") ? client.uuid : generateRandomShadowsocksPassword();
    const caption =
      `📄 <b>ملف تكوين Shadowsocks JSON (${escapeHtml(client.name || "")})</b>\n\n` +
      `🌐 <b>الهوست (Host):</b> <code>${escapeHtml(domain)}</code>\n` +
      `🎯 <b>التمويه (SNI / Fake Address):</b> <code>YouTube.com</code> / <code>google.com</code>\n` +
      `🔑 <b>كلمة المرور:</b> <code>${escapeHtml(password)}</code>\n` +
      `📍 <b>المسار (Path):</b> <code>${escapeHtml(client.path || "/ss")}</code>\n\n` +
      `💡 هذا الملف جاهز ومطابق لتكوين V2Ray / Xray مع دومين المشروع الحالي.`;

    const docRes = await sendTelegramDocumentBuffer(
      chatId,
      Buffer.from(jsonStr, "utf8"),
      `${safeName}_config.json`,
      caption
    );
    if (!docRes || !docRes.ok) {
      addLog(`sendShadowsocksJsonFile: sendDocument did not confirm success for chat ${chatId}`);
    }
  } catch (err: any) {
    addLog(`Failed to build/send Shadowsocks JSON config file: ${err?.message || err}`);
  }
}
...
function generateClientLinks(client: ClientConfig): { link: string; type: string } {
  ...
  } else if (client.protocol === "shadowsocks") {
    const password = (client.uuid && client.uuid !== "@dxuser") ? client.uuid : generateRandomShadowsocksPassword();
    const method = "aes-128-gcm";
    const userInfo = Buffer.from(`${method}:${password}`).toString("base64");
    const link = `ss://${userInfo}@${domain}:443?type=ws&security=tls&host=${encodeURIComponent(domain)}&sni=YouTube.com&path=${encodeURIComponent(clientPath)}#${encodeURIComponent(name)}`;
    return { link, type: "Shadowsocks" };
  } else {
    // VLESS
...
function findClientFromInput(text: string, clients: ClientConfig[]): ClientConfig | undefined {
  const trimmed = text.trim();
  if (!trimmed) return undefined;

  // 1. Exact match by name
  let found = clients.find(c => c.name === trimmed);
  if (found) return found;

  // 2. Match by stripping leading status emoji(s) (🟢, 🔴, ⏳, 🚫, 🔌, etc.)
  const stripped = trimmed.replace(/^[\p{Extended_Pictographic}\u{1F300}-\u{1F9FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\s]+/u, "").trim();
  if (stripped) {
    found = clients.find(c => c.name === stripped);
    if (found) return found;
  }

  // 3. Match against formatted keyboard button text (e.g. "🟢 Default Client (VMess)")
  found = clients.find(c => {
    ...
    return trimmed === `${statusIcon} ${c.name}` || trimmed === `${statusIcon}${c.name}`;
  });
  if (found) return found;

  // 4. Loose substring match if the client name is in the text
  found = clients.find(c => c.name.length >= 3 && (trimmed.includes(c.name) || (stripped && stripped.includes(c.name))));
  if (found) return found;
...
const PROTO_REPLY_KEYBOARD = {
  keyboard: [
    [{ text: "⚡ VLESS" }, { text: "🚀 VMess" }],
    [{ text: "🛡️ Trojan" }, { text: "🗝️ Shadowsocks" }],
...
async function handleTelegramUpdate(update: any) {
  ...
  if (update.message) {
    ...
    if (text.includes("إرسال ملف JSON") || text === "/json" || textLower === "json") {
      const selectedId = session.selectedClientId;
      if (selectedId) {
        const clients = getPersistedClients();
        const cli = clients.find(c => c.id === selectedId);
        if (cli) {
          await sendShadowsocksJsonFile(chatId, cli);
          return;
        }
      }
    }

    if (text.includes("ملف DarkTunnel") || text === "/darktunnel") {
      const selectedId = session.selectedClientId;
      if (selectedId) {
        const clients = getPersistedClients();
        const cli = clients.find(c => c.id === selectedId);
    ...
    if (session.action === "add_client_name") {
      ...
      const uuid = isSS ? generateRandomShadowsocksPassword() : crypto.randomUUID();
      ...
      const caption = `✅ <b>تم إنشاء التكوين بنجاح!</b>\n\n` +
        ...
        `⚡ <b>البروتوكول:</b> ${type}${isSS ? ' (aes-128-gcm)' : ''}\n` +
        `🌐 <b>الهوست (Host):</b> <code>${escapeHtml(domain)}</code>\n` +
        (isSS ? `🎯 <b>التمويه (SNI / Fake Address):</b> <code>YouTube.com</code> / <code>google.com</code>\n` : `🌐 <b>SNI:</b> <code>${escapeHtml(domain)}</code>\n`) +
        `🔑 <b>${isSS ? 'كلمة المرور' : 'UUID/Pass'}:</b> <code>${escapeHtml(newClient.uuid)}</code>\n` +
        `📍 <b>المسار:</b> <code>${escapeHtml(newClient.path)}</code>\n` +
      ...
      await sendAppConfigZip(chatId, newClient);
      if (newClient.protocol === "shadowsocks") {
        await sendShadowsocksJsonFile(chatId, newClient);
      }
      return;
    }

    if (session.action === "add_client_proto") {
      let proto: ClientConfig["protocol"] = "vless";
      if (text.includes("VMess") || textLower === "vmess" || text.includes("🚀")) proto = "vmess";
      else if (text.includes("Trojan") || textLower === "trojan" || text.includes("🛡️")) proto = "trojan";
      else if (text.includes("Shadowsocks") || textLower === "shadowsocks" || textLower === "ss" || text.includes("🗝️")) proto = "shadowsocks";
    ...
    // Check if user clicked a client button from the Reply Keyboard or sent a client name
    const allClients = getPersistedClients();
    const matchedClient = findClientFromInput(text, allClients);
    if (matchedClient) {
      await sendClientDetails(chatId, matchedClient.id);
      return;
    }

    // Direct reply keyboard selections for protocol selection (exact button labels only)
    if (text === "⚡ VLESS" || text === "🚀 VMess" || text === "🛡️ Trojan" || text === "🗝️ Shadowsocks" ||
        text === "/vless" || text === "/vmess" || text === "/trojan" || text === "/ss" || text === "/shadowsocks" ||
        textLower === "vless" || textLower === "vmess" || textLower === "trojan" || textLower === "shadowsocks" || textLower === "ss") {
      let proto: ClientConfig["protocol"] = "vless";
      if (text.includes("VMess") || textLower === "vmess" || text.includes("🚀")) proto = "vmess";
      else if (text.includes("Trojan") || textLower === "trojan" || text.includes("🛡️")) proto = "trojan";
      else if (text.includes("Shadowsocks") || textLower === "shadowsocks" || textLower === "ss" || text.includes("🗝️")) proto = "shadowsocks";
...
async function sendClientDetails(chatId: number | string, clientId: string) {
  ...
  const isSS = cli.protocol === "shadowsocks";
  const caption = `👤 <b>تفاصيل العميل: ${escapeHtml(cli.name)}</b>\n\n` +
    ...
    (locationDetail ? `${locationDetail}\n` : "") +
    `🔌 <b>حالة الاتصال:</b> ${connectionStatusText}\n` +
    `🔑 <b>${isSS ? 'كلمة المرور (Pass)' : 'UUID/Pass'}:</b> <code>${escapeHtml(cli.uuid)}</code>\n` +
  ...
  const keyboardRows: any[] = [];
  if (isSS) {
    keyboardRows.push([{ text: "📄 إرسال ملف JSON" }, { text: "📦 ملف DarkTunnel" }]);
  }
  keyboardRows.push([{ text: cli.enabled ? "⏸️ إيقاف العميل" : "▶️ تفعيل العميل" }, { text: "❌ حذف العميل" }]);
  keyboardRows.push([{ text: "📋 قائمة العملاء" }, { text: "🏠 القائمة الرئيسية" }]);
...
async function setupTelegramWebhook(attempt = 1): Promise<void> {
  if (!hasActiveBotToken()) {
    addLog("[Webhook] No valid Telegram bot token configured yet. Set TELEGRAM_BOT_TOKEN in .env or configure via setup wizard at /.");
    return;
  }
  ...
  if (res && res.ok) {
    ...
    if (
      ...
      desc.toLowerCase().includes("invalid token")
    ) {
      addLog(`[Webhook] Telegram Bot Token is invalid or not recognized by Telegram (${desc}). Webhook registration cancelled. Configure a valid TELEGRAM_BOT_TOKEN or complete the setup wizard at /.`);
      return;
    }
    if (attempt < 5) {
      addLog(`[Webhook] Failed to register webhook (${desc}) -- retrying in 10s (attempt ${attempt}/5)...`);
      setTimeout(() => setupTelegramWebhook(attempt + 1), 10000);
    } else {
      addLog(`[Webhook] Webhook registration stopped after ${attempt} attempts (${desc}).`);
    }
  }
}
...
server.on("upgrade", (req, socket, head) => {
  ...
  if (matchingClient) {
    if (matchingClient.protocol === "vmess") {
      ...
      req.url = "/by_moon_vmess";
    } else if (matchingClient.protocol === "trojan") {
      targetPort = ports.trojan;
      req.url = "/by_moon_trojan";
    } else if (matchingClient.protocol === "shadowsocks") {
      targetPort = getShadowsocksPortForClient(proxiedSlot, matchingClient.id, getPersistedClients());
      req.url = matchingClient.path || "/ss";
    } else {
      targetPort = ports.vless;
      req.url = "/by_moon";
    }
    ...
    targetPort = ports.trojan;
  } else if (pathname === "/ss" || pathname.startsWith("/ss") || url.includes("shadowsocks")) {
    targetPort = ports.shadowsocks;
    req.url = "/ss";
  }

  // Track per-client traffic. On the shared default paths (/by_moon, /by_moon_vmess,
  // /by_moon_trojan, /ss) several clients share the same inbound, so the individual
  // client is only known once we sniff the UUID out of the first VLESS frame.
  let detectedClientId: string | null = matchingClient ? matchingClient.id : (pathname === "/ss" || pathname.startsWith("/ss") ? "default-shadowsocks" : null);
  let uuidParsed = matchingClient ? matchingClient.protocol !== "vless" : (detectedClientId === "default-shadowsocks");
...
app.get("/config/shadowsocks", (req, res) => {
  const clients = getPersistedClients();
  const id = req.query.id as string;
  let cli = id ? clients.find(c => c.id === id) : clients.find(c => c.protocol === "shadowsocks");
  if (!cli) {
    cli = {
      id: "default-shadowsocks",
      name: "Default Client (Shadowsocks)",
  ...
  const jsonStr = generateShadowsocksClientJson(cli);
  res.setHeader("Content-Type", "application/json; charset=utf-8");
  res.setHeader("Content-Disposition", `attachment; filename="${cli.name}_shadowsocks.json"`);
  res.send(jsonStr);
});

app.get(["/config/shadowsocks.dark", "/config/darktunnel/shadowsocks"], (req, res) => {
  const clients = getPersistedClients();
  const id = req.query.id as string;
  let cli = id ? clients.find(c => c.id === id) : clients.find(c => c.protocol === "shadowsocks");
  if (!cli) {
  ...
  const darkLink = buildDarkTunnelLink(cli);
  res.setHeader("Content-Type", "application/octet-stream; charset=utf-8");
  res.setHeader("Content-Disposition", `attachment; filename="shadowsocks.dark"`);
  res.send(darkLink);
});
